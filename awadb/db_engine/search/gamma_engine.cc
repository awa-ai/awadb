...
// LOG(INFO) << "search request:" << RequestToString(request);
  ...
  /*
  std::cout<<"SearchResult size is "<<response_results.Results().size()<<std::endl;
  for (auto &iter : response_results.Results())  {
    std::cout<<"total is "<<iter.total<<std::endl;
    for (auto &iter_item : iter.result_items)  {
      std::cout<<"score is "<<iter_item.score<<std::endl;
      for (int i = 0; i < iter_item.names.size(); i++)  {
        std::cout<<"field name is "<<iter_item.names[i]<<"---value is "<<iter_item.values[i]<<std::endl;
      } 
    } 
  }*/ 
...
int GammaEngine::CreateTable(const std::string &table_str) {
  TableInfo table;
  table.SetName(table_str);
  //FieldInfo key_field;
  //key_field.name = "_id"; 
...
int GammaEngine::AddOrUpdate(Doc &doc) {
  ...
  if (!is_field_added)  {
    for (auto &iter : fields_table)  {
      int ret = table_->AddField(iter.name, iter.datatype, false);
      if (-1 == ret)  {
        LOG(ERROR)<<"Add new field "<<iter.name<<" error!";
        return ret; 
      }
    }

    for (auto &iter : fields_vec)  {
      int ret = table_->AddField(iter.name, iter.datatype, false);
      if (-1 == ret)  {
        LOG(ERROR)<<"Add new field "<<iter.name<<" error!";
        return ret; 
    ...
    if (table_->InitStorageMgr() != 0)  return -8;  
  } */
