...
void *Init(const char *log_dir, const char *index_path) {
  SetLogDictionary(log_dir);

  const std::string &path = std::string(index_path);
...
int SetLogDictionary(const char *dir) {
  if (!utils::isFolderExist(dir)) {
  ...
  defaultConf.setGlobally(el::ConfigurationType::MaxLogFileSize,
                          "209715200");  // 200MB
  defaultConf.setGlobally(el::ConfigurationType::Filename, std::string(dir) + "/gamma.log");
